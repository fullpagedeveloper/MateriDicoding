package com.fullpagedeveloper.myservice
...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btn_str_service.setOnClickListener (this)
        btn_str_intent_service.setOnClickListener (this)
        btn_str_bound_service.setOnClickListener (this)
        btn_stop_bound_service.setOnClickListener (this)
    }

    override fun onClick(v: View) {
        when(v.id){
            R.id.btn_str_service -> {
                val mStartServiceIntent = Intent(this@MainActivity, MyService::class.java)
                startService(mStartServiceIntent)
            }
            R.id.btn_str_intent_service -> {
                val mStartIntentService = Intent(this@MainActivity, MyIntentService::class.java)
                mStartIntentService.putExtra(MyIntentService.EXTRA_DURATION, 7000L)
                startService(mStartIntentService)
            }
            R.id.btn_str_bound_service -> {
